...
class handDetector():
    def __init__(self, mode=False, maxHands=2, modelComplexity=0, detectionCon=0.7, trackCon=0.7):
        """
        Initialize hand detector with optimized defaults for smooth tracking.
        
        Args:
            mode: Static image mode (False for video)
            maxHands: Maximum number of hands to detect
            modelComplexity: 0=Lite (fastest), 1=Full (more accurate)
            detectionCon: Minimum detection confidence (higher = less jittery)
            trackCon: Minimum tracking confidence (higher = more stable)
        """
        self.mode = mode
